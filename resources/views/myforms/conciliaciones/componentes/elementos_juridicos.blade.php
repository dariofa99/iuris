<div class="card card-outline card-info" id="elementos_juridicos">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    <label style="display: block; margin-bottom:10px ml-3">
                        Hechos
                        @if (currentUser()->hasRole('diradmin') ||
                                currentUser()->hasRole('coord_centro_conciliacion') ||
                                currentUser()->hasRole('amatai') ||
                                (currentUserInConciliacion($conciliacion->id, ['solicitante', 'auxiliar', 'conciliador']) ||
                                    currentUser()->hasRole('amatai')))
                            @if (
                                ...
                                <button type="button" data-tipo="206"
                                    class="btn btn-primary btn-sm float-right btn_add_conc_he_con">Agregar
                                    hechos</button>
                            @endif
                        @endif
                    </label>
                        ...
                        @if (currentUser()->hasRole('diradmin') ||
                            ...
                            @if (
                                $conciliacion->estado_id == 174 ||
                                    ...
                                    $conciliacion->estado_id == 194 ||
                                    ($conciliacion->estado_id == 240 and currentUserInConciliacion($conciliacion->id, ['autor', 'solicitante'])))
                                <button type="button" data-tipo="207"
                                    class="btn btn-primary btn-sm float-right btn_add_conc_he_con">
                                    Agregar pretensión</button>
                            @endif
                        @endif
                    </label>
