    ...
    @if ($expediente->expestado_id == 5)
        ...
        @else
            <button type="button" data-estado="{{ $expediente->expestado_id }}" class="btn btn-warning btn-sm mb-2"
                ...
                Volver a evaluar con nota mínima y cerrar caso
            </button>
        @endif
    @elseif(
...
{{-- @if ($expediente->getDocenteAsig()->idnumber == currentUser()->idnumber or currentUser()->hasRole('estudiante') or currentUser()->hasRole('amatai') or currentUser()->hasRole('diradmin') or currentUser()->hasRole('dirgral'))
    <div class="col-md-12" align="right">
        ...
        @if ((currentUser()->hasRole('estudiante') and $expediente->getDaysOrColorForClose('dias') >= 10 and $expediente->estado->id == 1 || $expediente->estado->id == 3) || ($expediente->getDocenteAsig()->idnumber == currentUser()->idnumber and $expediente->estado->id == 4) || (currentUser()->hasRole('amatai') or currentUser()->hasRole('diradmin') or currentUser()->hasRole('dirgral')))
        ...
        @if ($expediente->expestado_id == 5 and $expediente->exptipoproce_id == 1 and $expediente->getDocenteAsig()->idnumber == currentUser()->idnumber || currentUser()->hasRole('diradmin') || currentUser()->hasRole('amatai') || currentUser()->hasRole('dirgral'))
        @elseif(
            $expediente->exptipoproce_id == 1 and
                $expediente->expestado_id == 1 || $expediente->expestado_id == 3 || $expediente->expestado_id == 4 and
        ...
        @if ($expediente->expestado_id != 2 and $expediente->expestado_id != 5 and currentUser()->hasRole('dirgral') || currentUser()->hasRole('amatai') || currentUser()->hasRole('diradmin'))
            <button type="button" class="btn btn-danger btn-sm mb-2" id="btn_cerrar_dr_caso">
                Cerrar caso sin evaluar
            </button>
        @endif

        @if ($expediente->expestado_id != 2 and $expediente->expestado_id != 5 and $expediente->expestado_id != 7 and currentUser()->hasRole('dirgral') || currentUser()->hasRole('amatai') || currentUser()->hasRole('diradmin') || currentUser()->hasRole('coordprac'))
