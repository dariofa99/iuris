...
var myDropzone_log = new Dropzone("div#cont_files", { // Make the whole body a dropzone
  url: "/solicitudes/store/documento", // Set the url
  thumbnailWidth: 80,
  thumbnailHeight: 80,
  paramName: "solicitud_file",
  parallelUploads: 20,
...
myDropzone_log.on("sending", function (file, xhr, formData) {
  ...
  formData.append("concept", $(file.previewElement.querySelector("input")).val());
   formData.append("type_category_id", 163);
  formData.append("solicitud_id", $("#solicitud_id").val())
  formData.append("view_template", "myforms.recepcion.expedientes.chat.files_ajax") 
...
myDropzone_log.on("success", function (file, response) {
  ...
  if (response.view || response.view == "") {
    $("#tblListFilesShared tbody").html(response.view);
