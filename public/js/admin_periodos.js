...
$(document).ready(function () { 
    ...
    $("#myform_create_periodo").on("submit", async function (e) {
       ...
       var errors = validateForm("myform_create_periodo");
        if (errors.length <= 0) {
            ...
            let response = await periodoService.store(data);
            if (response.errors) {
    ...
    $("#table_list_model").on("change", ".radio_state_periodo",async function () {
        var id = $(this).attr("data-id");
        $("#wait").show();
        let response = await periodoService.changeState(id);
        $("#table_list_model").html(response);
        $("#wait").hide();
    });
    $("#table_list_model").on("click", ".btn_edit_per",async function () {
        var id = $(this).attr("data-id");
        $("#wait").show();
        let res = await periodoService.edit(id);
    ...
    $("#myform_edit_periodo").submit(async function (e) {
       ...
       var errors = validateForm("myform_edit_periodo");
        if (errors.length <= 0) {
            ...
            $("#wait").show();
            let response = await periodoService.update(data,data.id_periodo);
            if (response.errors) {
                response.errors.forEach(error => {
                  toastr.error(error, "", {
                    positionClass: "toast-top-right",
