...
class Requerimiento extends Model
{
    ...
    public function scopeCriterio($query, $request)
    {
        $data = $request->data;
        $criterio = $request->tipo_busqueda;
        if (trim($data) != '') {
            ...
            switch ($criterio) {
                case 'codido_exp':
                    ...
                    break;
                case 'fecha_creacion':
                    return $query->whereDate('requerimientos.created_at', $data);
                    break;
                case 'fecha_cita':
