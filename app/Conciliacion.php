...
class Conciliacion extends Model
{
    ...
    {
        return $this->getPersonaExternaForm()
    ...
    {
        return $this->aditional_static_data()
            ->whereHas('reference', function ($q) use ($question) {
    ...
    public function getUserQueForm($parte, $section)
    {
        ...
        $referece_data = ReferencesData::join('conciliacion_user_form  as rd', 'rd.reference_data_id', '=', 'references_data.id')
            ...
            ->select(
                'references_data.id as id',
                'references_data.name as name',
                'references_data.type_data_id as type_data_id',
                'references_data.type_data_id as type_data_id',
                'short_name'
            )
            ->where('parte', $parte)
