$(function () {
  ...
  var channel = Echo.channel('recepcionDocumentSolicitud');

  channel.listen('.recepcion-documentSolicitud', function (data) {
    var myTk = $("#solicitudTk").val()

    if (myTk == data.data.token) {
      toastr.success("Se agregó un nuevo documento", "", {
        timeOut: "4000",
      });
      if (data.data.view || data.data.view == "") {
        $("#tblListFilesShared tbody").html(data.data.view);
      }
    }
  });
  ...
  $("#btn_acept_solicitud").on("click", function (e) {
    ...
    Swal.fire({
      ...
      text: "Se habilitará el registro de usuario!",
      type: 'info',
  ...
  $("#btn_denied_solicitud").on("click", function (e) {
    ...
    var id = $(this).attr('data-id');
    Swal.fire({
      title: 'Rechazando la solicitud',
      text: "!Atención!",
      input: 'textarea',
      inputPlaceholder: '¿Por qué va a rechazar la solicitud?',
      inputAttributes: {
      ...
      preConfirm: (text) => {
        if (text !== '') {
          $("#wait").show();
          let request = { 'type_status_id': 157 ,"mensaje" : text }
          updateSolicitud(request, id)
