...
<div style="display: {{ (isset($user) and $user->pbepersondiscap == 0) || !isset($user) ? 'none' : 'block' }}"
    ...
    <div class="has_apoyo form-group alert alert-info" style="display: {{ (isset($user) and $user->has_apoyo == 0) || !isset($user) ? 'none' : 'block' }}">
        Obligaciones y consideraciones relacionadas con la privacidad en el trámite de apoyo a personas con
        discapacidad por parte de la persona que brinda el apoyo. Es fundamental que estas pautas sean
        comprendidas y respetadas para garantizar el bienestar y la autonomía del solicitante.

        <ol>
            <li><strong> Confidencialidad y Privacidad:</strong><br>
                Se compromete a respetar y tratar con la máxima reserva toda la información personal, médica o
                confidencial relacionada con el solicitante y que eventualmente pueda conocer. Igualmente tenga
                en cuenta que no está facultado(a) para compartir información privada con terceros sin el
                consentimiento expreso del solicitante.

            </li>
            <li><strong>Respeto a la Autonomía:</strong><br>
                Se compromete a respetar la autonomía del solicitante permitiéndole tomar sus propias decisiones,
                teniendo en cuenta que el apoyo brindado hace parte de los ajustes razonables que se generan en
                favor del usuario.
        ...
        <div>
            <input {{ (isset($user) and $user->has_apoyo != 1) ?: 'checked' }} type="checkbox" disabled required
                class="required" name="acept_ter" id="acept_ter">
