...
(function (exports) {
  ...
  var MP3Recorder = function (config) {
    ...
    config = config || {};
    var realTimeWorker = new Worker('/plugins/node/worker-realtime.js');
    ...
    realTimeWorker.onmessage = function (e) {
      switch (e.data.cmd) {
        case 'end':
          ...
          $('#estado-voz').text('enviando...');
          //console.log('MP3 data size', e.data.buf.length);
          break;
        case 'error':
          if (currentErrorCallback) {
           $('#estado-voz').text('error');
           alert('Error al procesar la grabación: '+e.data.error);
            restaurar();
